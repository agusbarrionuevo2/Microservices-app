...
export const getAllPlanets = () => {
	return async function (dispatch) {
		...
		return dispatch({type: GET_ALL_PLANETS, payload: allPlanets.data})
	}
}
...
export const getPlanet = (id) => {
	return async function(dispatch){
		...
		return dispatch({type: GET_PLANET, payload: planetById.data})
	}
}
...
export const getAllCharacters = () => {
	return async function (dispatch) {
		...
		return dispatch({type: GET_ALL_CHARACTERS, payload: allCharacters.data})
	}
}

export const getCharacteer = (id) => {
	return async function(dispatch){
		const characterById = await axios.get(`http://localhost:8000/characters/${id}`)
		return dispatch({type: GET_CHARACTER, payload: characterById.data})
	}
}
...
export const getAllFilms = () => {
	return async function(dispatch){
		...
		return dispatch({type: GET_ALL_FILMS, payload: allFilms.data})
	}
}
...
export const getFilm = (id) => {
	return async function(dispatch){
		...
		return dispatch({type: GET_FILM, payload: filmById.data})
	}
}
